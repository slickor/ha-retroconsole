...
cd "$GAMEDIR" # Change to the script's directory

# Ensure pip is installed for python3...
# Check if pip is already available before trying to bootstrap it
if ! python3 -m pip --version > /dev/null 2>&1; then
    echo "pip not found, trying to bootstrap..." # pip not found, trying to bootstrap...
    if python3 -m ensurepip --upgrade > /dev/null 2>&1; then
        echo "Successfully bootstrapped pip." # Successfully bootstrapped pip.
    else
        echo "---------------------------------------------------------------------------"
        echo "ERROR: 'pip' is missing and your OS (ArkOS?) does not support 'ensurepip'." # ERROR: 'pip' is missing and your OS (ArkOS?) does not support 'ensurepip'.
        echo "" # Empty line
        echo "To fix this without SSH:" # To fix this without SSH:
...
# Installing Python dependencies into local libs folder...
...
if python3 -m pip install --target="./libs" --no-cache-dir --no-compile "websocket-client<1.4.0" "requests<2.32.0" "urllib3<2.0.0" -r requirements.txt; then
    echo "Cleaning up metadata and binaries..." # Cleaning up metadata and binaries...
    ...
    echo "Setup complete!" # Setup complete!
    touch .installed
else
    echo "Installation failed. Check internet connection." # Installation failed. Check internet connection.
    exit 1
fi